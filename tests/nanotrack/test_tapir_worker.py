...
from nanotrack.trackers.run_tapir_subprocess import (
    _build_stub_output,
    _frames_to_rgb_uint8,
    _resolve_inference_shape,
    _scale_query_points_tyx,
)


class TapirWorkerHelperTests(unittest.TestCase):
    ...
    def test_scale_query_points_tyx_scales_only_spatial_coordinates(self) -> None:
        query_points = np.asarray([[1.0, 10.0, 20.0], [3.0, 5.0, 7.5]], dtype=np.float32)

        scaled = _scale_query_points_tyx(query_points, source_hw=(20, 40), target_hw=(10, 80))

        np.testing.assert_allclose(
            scaled,
            np.asarray([[1.0, 5.0, 40.0], [3.0, 2.5, 15.0]], dtype=np.float32),
        )
    ...
    def test_build_stub_output_keeps_points_fixed_and_visible_after_seed(self) -> None:
        ...
        output = _build_stub_output(run_input, model_name="tapir_stub", checkpoint_name=None)

        self.assertEqual(output.model_name, "tapir_stub")
...
class TapirWorkerScriptTests(unittest.TestCase):
    def test_worker_script_runs_stub_mode_without_checkpoint(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result.model_name, "tapir_stub")
