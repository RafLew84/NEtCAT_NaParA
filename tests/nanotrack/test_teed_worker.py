...
from nanotrack.edges.run_teed_subprocess import (
    _frame_to_rgb_uint8,
...
class TeedWorkerHelperTests(unittest.TestCase):
    ...
    def test_prepare_frame_tensor_crops_roi_without_dividing_by_255(self) -> None:
        polygon_mask = np.zeros((6, 8), dtype=bool)
        polygon_mask[1:5, 2:7] = True
        bbox = _resolve_polygon_bbox(polygon_mask)
        ...
        self.assertGreater(np.max(chw_frame), 1.0)
