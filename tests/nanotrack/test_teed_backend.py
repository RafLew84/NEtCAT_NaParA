...
class TeedSubprocessBackendTests(unittest.TestCase):
    ...
    def test_backend_roundtrip_executes_worker_and_parses_output(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            _write_worker_script(
                ...
                assert args.checkpoint == "teed_checkpoint.pth"
                assert args.repo_path == "teed-repo"
                ...
                np.savez_compressed(
                    ...
                    model_name=np.asarray("teed"),
                    checkpoint_name=np.asarray("5_model.pth"),
                )
                """,
            )

            backend = TeedSubprocessBackend(
                TeedBackendConfig(
                    python_executable=sys.executable,
                    worker_script=worker_script,
                    checkpoint_path="teed_checkpoint.pth",
                    repo_path="teed-repo",
                    device="cuda:0",
                    timeout_sec=5.0,
            ...
            self.assertEqual(result.model_name, "teed")
            self.assertEqual(result.checkpoint_name, "5_model.pth")
    ...
    def test_backend_raises_on_nonzero_worker_exit(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            _write_worker_script(
                ...
                """
                import sys
                sys.stderr.write("teed exploded\\n")
                raise SystemExit(4)
                """,
            )
            ...
            with self.assertRaises(TeedBackendError) as exc_info:
                backend.run(self._make_run_input())

            self.assertIn("Exit code: 4", str(exc_info.exception))
            self.assertIn("teed exploded", str(exc_info.exception))
    ...
    def test_backend_raises_timeout_error(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(TeedBackendTimeoutError) as exc_info:
                backend.run(self._make_run_input())
    ...
    def test_backend_config_validates_timeout(self) -> None:
        with self.assertRaises(ValueError):
            TeedBackendConfig(timeout_sec=0.0)
