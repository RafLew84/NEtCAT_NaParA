...
class AdjacentPhaseRegistrationTests(unittest.TestCase):
    ...
    def test_batch_registration_skips_excluded_frames_for_pair_estimation(self) -> None:
        class RecordingBackend:
            ...
            def estimate_pair(
                ...
                *,
                ...
                self.pairs.append((reference_index, moving_index))
                return RegistrationFrameResult(
                    frame_index=moving_index,
                    shift_xy=(1.0, 0.0),
                    method="recording_backend",
                    quality_score=0.9,
                    status="ok",
                )
        ...
        result_set = run_adjacent_phase_registration(
            np.stack([frame, frame, frame, frame, frame]).astype(np.float32),
            backend=backend,
            settings=RegistrationSettings(registration_view="normalized"),
            excluded_frame_indices=[1, 3],
        )

        self.assertEqual(backend.pairs, [(0, 2), (2, 4)])
        self.assertEqual(result_set.reference_frame_index, 0)
        self.assertEqual(result_set.template_frame_indices, (0,))
        self.assertEqual(result_set.frame_indices, [0, 1, 2, 3, 4])
        np.testing.assert_allclose(
            result_set.shifts_xy_array(),
            np.asarray(
                [
        ...
        self.assertEqual(result_set.get_result(1).method, "excluded_frame")
        self.assertEqual(result_set.get_result(1).status, "manual_review")
        self.assertEqual(result_set.get_result(3).method, "excluded_frame")
        self.assertEqual(result_set.status_counts()["manual_review"], 2)
    ...
    def test_rejects_invalid_inputs_and_reference_strategy(self) -> None:
        ...
        with self.assertRaises(ValueError):
            run_adjacent_phase_registration(
                ...
                excluded_frame_indices=[0, 1],
            )
        with self.assertRaises(IndexError):
            run_adjacent_phase_registration(frame[None, :, :], excluded_frame_indices=[1])
