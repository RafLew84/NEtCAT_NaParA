#!/usr/bin/env python3
"""Run real DDN inference for NanoTrack in a dedicated interpreter."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NanoTrack DDN subprocess worker.")
    ...
    parser.add_argument("--checkpoint", required=True, help="Path to a DDN checkpoint.")
    parser.add_argument("--config", required=True, help="Path to a DDN YAML config.")
    parser.add_argument("--backbone", required=True, help="Path to a CAFormer backbone checkpoint.")
    parser.add_argument("--repo-path", default=None, help="Optional path to the local DDN repo.")
...
def _resolve_repo_root(repo_path: str | Path | None) -> Path:
    ...
    model_path = candidate / "model" / "sigma_logit_unet.py"
    if not model_path.exists():
        raise FileNotFoundError(f"DDN repo does not contain model/sigma_logit_unet.py: {model_path}")
    return candidate
...
def _load_yaml_config(config_path: Path, backbone_path: Path) -> dict:
    import yaml

    with config_path.open("r", encoding="utf-8") as stream:
        config = yaml.safe_load(stream)
    if not isinstance(config, dict):
        raise TypeError("DDN config must be a mapping.")
    config.setdefault("ckpt", {})
    config["ckpt"]["caformer_m36"] = str(backbone_path)
    return config


def _load_ddn_model_class(repo_root: Path):
    sys.path.insert(0, str(repo_root))
    try:
        module = importlib.import_module("model.sigma_logit_unet")
    finally:
        if sys.path and sys.path[0] == str(repo_root):
            sys.path.pop(0)
    if not hasattr(module, "Mymodel"):
        raise ImportError(f"Mymodel class not found in {repo_root / 'model' / 'sigma_logit_unet.py'}")
...
def _extract_state_dict(torch_module, checkpoint_path: Path) -> dict:
    ...
    if not isinstance(checkpoint, dict):
        raise TypeError("DDN checkpoint must contain a state_dict mapping.")
...
def _load_model(
    ...
    model_args = SimpleNamespace(encoder="DDN-M36", cfg=config, distribution="gs")
    Mymodel = _load_ddn_model_class(repo_root)
...
def _normalize_prob_map(edge_prob: np.ndarray) -> np.ndarray:
    edge_f32 = np.asarray(edge_prob, dtype=np.float32)
    value_min = float(np.min(edge_f32))
    value_max = float(np.max(edge_f32))
    if value_max <= value_min + 1e-8:
        return np.zeros(edge_f32.shape, dtype=np.float32)
    return np.asarray((edge_f32 - value_min) / (value_max - value_min), dtype=np.float32)
