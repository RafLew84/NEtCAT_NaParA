#!/usr/bin/env python3
"""Run real TEED inference for NanoTrack in a dedicated interpreter."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NanoTrack TEED subprocess worker.")
    ...
    parser.add_argument("--checkpoint", required=True, help="Path to a TEED checkpoint.")
    parser.add_argument("--repo-path", default=None, help="Optional path to the local TEED repo.")
...
def _resolve_repo_root(repo_path: str | Path | None) -> Path:
    ...
    model_path = candidate / "ted.py"
    if not model_path.exists():
        raise FileNotFoundError(f"TEED repo does not contain ted.py: {model_path}")
    return candidate


def _load_teed_class(repo_root: Path):
    ted_path = repo_root / "ted.py"
    spec = importlib.util.spec_from_file_location("nanotrack_teed_model", ted_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load TEED module from {ted_path}")
    ...
    try:
        ...
        if sys.path and sys.path[0] == str(repo_root):
            sys.path.pop(0)
    if not hasattr(module, "TED"):
        raise ImportError(f"TED class not found in {ted_path}")
    return module.TED
...
def _load_model(repo_root: Path, checkpoint_path: Path, requested_device: str):
    ...
    TED = _load_teed_class(repo_root)
    model = TED().to(device)
...
def _prepare_frame_tensor(
    ...
    *,
    ...
    return chw, source_hw, target_hw
...
def _run_model_on_tensor(model, torch_module, device, chw_frame: np.ndarray) -> np.ndarray:
    ...
    with torch_module.no_grad():
        ...
        probs = [torch_module.sigmoid(pred[0, 0]) for pred in predictions]
        fused_prob = torch_module.stack(probs, dim=0).mean(dim=0)
    edge_prob = fused_prob.detach().cpu().numpy().astype(np.float32, copy=False)
    return edge_prob
