#!/usr/bin/env python3
"""Run real UAED inference for NanoTrack in a dedicated interpreter."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NanoTrack UAED subprocess worker.")
    ...
    parser.add_argument("--checkpoint", required=True, help="Path to a UAED checkpoint.")
    ...
    parser.add_argument("--distribution", default="gs", help="UAED output distribution mode.")
...
def _resolve_repo_root(repo_path: str | Path | None) -> Path:
    ...
    model_path = candidate / "model" / "sigma_logit_unetpp.py"
    if not model_path.exists():
        raise FileNotFoundError(f"UAED repo does not contain model/sigma_logit_unetpp.py: {model_path}")
    return candidate
...
def _extract_state_dict(torch_module, checkpoint_path: Path) -> dict:
    ...
    if not isinstance(checkpoint, dict):
        raise TypeError("UAED checkpoint must contain a state_dict mapping.")
    return _strip_module_prefix(dict(checkpoint))


def _load_uaed_model_class(repo_root: Path):
    sys.path.insert(0, str(repo_root))
    try:
        module = importlib.import_module("model.sigma_logit_unetpp")
    finally:
        if sys.path and sys.path[0] == str(repo_root):
            sys.path.pop(0)
    if not hasattr(module, "Mymodel"):
        raise ImportError(f"Mymodel class not found in {repo_root / 'model' / 'sigma_logit_unetpp.py'}")
...
def _load_model(
    ...
    *,
    ...
    Mymodel = _load_uaed_model_class(repo_root)
