#!/usr/bin/env python3
"""Run real PiDiNet inference for NanoTrack in a dedicated interpreter."""
...
PIDINET_RGB_MEAN = np.asarray([0.485, 0.456, 0.406], dtype=np.float32)
PIDINET_RGB_STD = np.asarray([0.229, 0.224, 0.225], dtype=np.float32)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NanoTrack PiDiNet subprocess worker.")
    ...
    parser.add_argument("--checkpoint", required=True, help="Path to a PiDiNet checkpoint.")
    parser.add_argument("--repo-path", default=None, help="Optional path to the local PiDiNet repo.")
    parser.add_argument("--model", default="pidinet_converted", help="PiDiNet model factory name.")
    parser.add_argument("--model-config", default="carv4", help="PiDiNet config name.")
    parser.add_argument("--sa", action="store_true", help="Enable CSAM, matching table5_pidinet.")
    parser.add_argument("--dil", action="store_true", help="Enable CDCM dilation, matching table5_pidinet.")
    parser.add_argument(
        "--evaluate-converted",
        action="store_true",
        help="Convert original PDC checkpoint weights for the converted PiDiNet model.",
    )
...
def _resolve_repo_root(repo_path: str | Path | None) -> Path:
    ...
    model_path = candidate / "models" / "pidinet.py"
    convert_path = candidate / "models" / "convert_pidinet.py"
    if not model_path.exists():
        raise FileNotFoundError(f"PiDiNet repo does not contain models/pidinet.py: {model_path}")
    if not convert_path.exists():
        raise FileNotFoundError(f"PiDiNet repo does not contain models/convert_pidinet.py: {convert_path}")
    return candidate
...
def _extract_state_dict(torch_module, checkpoint_path: Path) -> dict:
    ...
    if not isinstance(checkpoint, dict):
        raise TypeError("PiDiNet checkpoint must contain a state_dict mapping.")
    return dict(checkpoint)


def _load_pidinet_modules(repo_root: Path):
    sys.path.insert(0, str(repo_root))
    try:
        models_module = importlib.import_module("models")
        convert_module = importlib.import_module("models.convert_pidinet")
    finally:
        if sys.path and sys.path[0] == str(repo_root):
            sys.path.pop(0)
    return models_module, convert_module
...
def _load_model(
    ...
    *,
    ...
    use_sa: bool,
    use_dil: bool,
    evaluate_converted: bool,
):
    torch_module = _load_torch()
    device = _resolve_torch_device(torch_module, requested_device)
    _configure_cuda_runtime(torch_module, device)

    models_module, convert_module = _load_pidinet_modules(repo_root)
    if not hasattr(models_module, model_name):
        raise ImportError(f"PiDiNet model factory not found: models.{model_name}")

    model_args = SimpleNamespace(config=model_config, sa=bool(use_sa), dil=bool(use_dil))
    model = getattr(models_module, model_name)(model_args)
    state_dict = _extract_state_dict(torch_module, checkpoint_path)
    if evaluate_converted:
        state_dict = convert_module.convert_pidinet(state_dict, model_config)
...
def _resolve_inference_shape(
    ...
    return _round_up_to_multiple(height, 8), _round_up_to_multiple(width, 8)
...
def _prepare_frame_tensor(
    ...
    *,
    ...
    chw = (chw - PIDINET_RGB_MEAN[:, None, None]) / PIDINET_RGB_STD[:, None, None]
...
def _run_real_inference(
    ...
    *,
    ...
    model, torch_module, device = _load_model(
        ...
        use_sa=use_sa,
        use_dil=use_dil,
        evaluate_converted=evaluate_converted,
    )
...
def main() -> int:
    ...
    run_output = _run_real_inference(
        ...
        use_sa=bool(args.sa),
        use_dil=bool(args.dil),
        evaluate_converted=bool(args.evaluate_converted),
    )
