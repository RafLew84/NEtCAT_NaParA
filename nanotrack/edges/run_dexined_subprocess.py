#!/usr/bin/env python3
"""Run real DexiNed inference for NanoTrack in a dedicated interpreter."""
...
DEXINED_MEAN_BGR = np.asarray([103.939, 116.779, 123.68], dtype=np.float32)
...
def _resolve_repo_root(repo_path: str | Path | None) -> Path:
    ...
    if not model_path.exists():
        raise FileNotFoundError(f"DexiNed repo does not contain model.py: {model_path}")
    return candidate


def _load_dexined_class(repo_root: Path):
    model_path = repo_root / "model.py"
    spec = importlib.util.spec_from_file_location("nanotrack_dexined_model", model_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load DexiNed module from {model_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if not hasattr(module, "DexiNed"):
        raise ImportError(f"DexiNed class not found in {model_path}")
    return module.DexiNed
...
def _load_model(repo_root: Path, checkpoint_path: Path, requested_device: str):
    ...
    DexiNed = _load_dexined_class(repo_root)
...
def _resolve_inference_shape(
    ...
    return _round_up_to_multiple(height, 16), _round_up_to_multiple(width, 16)
...
def _prepare_frame_tensor(
    ...
    *,
    ...
    crop_rgb = np.asarray(crop_rgb, dtype=np.float32)
    crop_rgb -= DEXINED_MEAN_BGR
...
def _run_model_on_tensor(model, torch_module, device, chw_frame: np.ndarray) -> np.ndarray:
    ...
    with torch_module.no_grad():
        ...
        fused_logits = predictions[-1]
        fused_prob = torch_module.sigmoid(fused_logits)[0, 0]
